...
// Bangun HTML tabel ke dalam $data_message (pakai output buffering)
ob_start();
?>
<table>
    <thead>
        ...
        if (file_exists(FILE_PENDAFTARAN)) {
            ...
            if (!empty($pendaftar)) {
                foreach ($pendaftar as $data) {
                    // Aman-kan & pecah baris data
                    $parts = explode(';', $data);
                    $nama = htmlspecialchars(trim($parts[0] ?? ''), ENT_QUOTES, 'UTF-8');
                    $email = htmlspecialchars(trim($parts[1] ?? ''), ENT_QUOTES, 'UTF-8');
                    $tanggal_lahir = htmlspecialchars(trim($parts[2] ?? ''), ENT_QUOTES, 'UTF-8');
